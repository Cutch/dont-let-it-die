const itemsSprites = {
  sprites: {
    'bow-and-arrow': {
      options: { type: 'item', itemType: 'weapon', name: 'Bow And Arrow' },
      frame: { x: 0, y: 0, w: 438, h: 438 },
      rotate: 0,
    },
    'medical-hut': {
      options: { type: 'item', itemType: 'building', name: 'Medical Hut' },
      frame: { x: 438, y: 0, w: 438, h: 438 },
      rotate: 0,
    },
    'bone-club': { options: { type: 'item', itemType: 'weapon', name: 'Bone Club' }, frame: { x: 876, y: 0, w: 438, h: 438 }, rotate: 0 },
    'bone-scythe': {
      options: { type: 'item', itemType: 'weapon', name: 'Bone Scythe' },
      frame: { x: 1314, y: 0, w: 438, h: 438 },
      rotate: 0,
    },
    bag: { options: { type: 'item', itemType: 'tool', name: 'Bag' }, frame: { x: 1752, y: 0, w: 438, h: 438 }, rotate: 0 },
    'bone-armor': { options: { type: 'item', itemType: 'tool', name: 'Bone Armor' }, frame: { x: 2190, y: 0, w: 438, h: 438 }, rotate: 0 },
    'camp-walls': {
      options: { type: 'item', itemType: 'building', name: 'Camp Walls' },
      frame: { x: 0, y: 440, w: 438, h: 438 },
      rotate: 0,
    },
    fire: { options: { type: 'game-piece', name: 'Fire' }, frame: { x: 438, y: 440, w: 438, h: 438 }, rotate: 0 },
    'hide-armor': { options: { type: 'item', itemType: 'tool', name: 'Hide Armor' }, frame: { x: 876, y: 440, w: 438, h: 438 }, rotate: 0 },
    'knowledge-hut': {
      options: { type: 'item', itemType: 'building', name: 'Knowledge Hut' },
      frame: { x: 1314, y: 440, w: 438, h: 438 },
      rotate: 0,
    },
    skull: { options: { type: 'game-piece', name: 'Skull' }, frame: { x: 1756, y: 440, w: 434, h: 438 }, rotate: 0 },
    hatchet: { options: { type: 'item', itemType: 'tool', name: 'Hatchet' }, frame: { x: 2190, y: 440, w: 438, h: 438 }, rotate: 0 },
    club: { options: { type: 'item', itemType: 'weapon', name: 'Club' }, frame: { x: 0, y: 880, w: 438, h: 438 }, rotate: 0 },
    'cooking-hut': {
      options: { type: 'item', itemType: 'building', name: 'Cooking Hut' },
      frame: { x: 438, y: 880, w: 438, h: 438 },
      rotate: 0,
    },
    'carving-knife': {
      options: { type: 'item', itemType: 'tool', name: 'Carving Knife' },
      frame: { x: 876, y: 880, w: 438, h: 438 },
      rotate: 0,
    },
    'item-back': { options: { type: 'back', name: 'Item Back' }, frame: { x: 1314, y: 880, w: 438, h: 438 }, rotate: 0 },
    'sling-shot': {
      options: { type: 'item', itemType: 'weapon', name: 'Sling Shot' },
      frame: { x: 1752, y: 880, w: 438, h: 438 },
      rotate: 0,
    },
    'pick-axe': { options: { type: 'item', itemType: 'tool', name: 'Pick Axe' }, frame: { x: 2190, y: 880, w: 438, h: 438 }, rotate: 0 },
    'planning-hut': {
      options: { type: 'item', itemType: 'building', name: 'Planning Hut' },
      frame: { x: 0, y: 1320, w: 438, h: 438 },
      rotate: 0,
    },
    spear: { options: { type: 'item', itemType: 'weapon', name: 'Spear' }, frame: { x: 438, y: 1320, w: 438, h: 438 }, rotate: 0 },
    'sharp-stick': {
      options: { type: 'item', itemType: 'weapon', name: 'Sharp Stick' },
      frame: { x: 876, y: 1320, w: 438, h: 438 },
      rotate: 0,
    },
    shelter: { options: { type: 'item', itemType: 'building', name: 'Shelter' }, frame: { x: 1314, y: 1320, w: 438, h: 438 }, rotate: 0 },
    'rock-knife': {
      options: { type: 'item', itemType: 'weapon', name: 'Rock Knife' },
      frame: { x: 1752, y: 1320, w: 438, h: 438 },
      rotate: 0,
    },
    'stone-hammer': {
      options: { type: 'item', itemType: 'tool', name: 'Stone Hammer' },
      frame: { x: 2190, y: 1320, w: 438, h: 438 },
      rotate: 0,
    },
    'mortar-and-pestle': {
      options: { type: 'item', itemType: 'tool', name: 'Mortar And Pestle' },
      frame: { x: 0, y: 1760, w: 438, h: 438 },
      rotate: 0,
    },
    bandage: { options: { type: 'item', itemType: 'tool', name: 'Bandage' }, frame: { x: 438, y: 1760, w: 438, h: 438 }, rotate: 0 },
    'skull-shield': {
      options: { type: 'item', itemType: 'tool', name: 'Skull Shield' },
      frame: { x: 876, y: 1760, w: 438, h: 438 },
      rotate: 0,
    },
    'cooking-pot': {
      options: { type: 'item', itemType: 'tool', name: 'Cooking Pot' },
      frame: { x: 1314, y: 1760, w: 438, h: 438 },
      rotate: 0,
    },
    'bone-claws': {
      options: { type: 'item', itemType: 'tool', name: 'Bone Claws' },
      frame: { x: 1752, y: 1760, w: 438, h: 438 },
      rotate: 0,
    },
    'bone-flute': {
      options: { type: 'item', itemType: 'tool', name: 'Bone Flute' },
      frame: { x: 2190, y: 1760, w: 438, h: 438 },
      rotate: 0,
    },
    'stock-hut': {
      options: { type: 'item', itemType: 'building', name: 'Stock Hut' },
      frame: { x: 0, y: 2200, w: 438, h: 438 },
      rotate: 0,
    },
    whip: { options: { type: 'item', itemType: 'weapon', name: 'Whip' }, frame: { x: 438, y: 2200, w: 438, h: 438 }, rotate: 0 },
    'fire-stick': {
      options: { type: 'item', itemType: 'weapon', name: 'Fire Stick' },
      frame: { x: 876, y: 2200, w: 438, h: 438 },
      rotate: 0,
    },
    'rock-weapon': { options: { type: 'item', itemType: 'weapon', name: 'Rock' }, frame: { x: 1314, y: 2200, w: 438, h: 438 }, rotate: 0 },
    bola: { options: { type: 'item', itemType: 'weapon', name: 'Bola' }, frame: { x: 1752, y: 2200, w: 438, h: 438 }, rotate: 0 },
    boomerang: { options: { type: 'item', itemType: 'weapon', name: 'Boomerang' }, frame: { x: 2190, y: 2200, w: 438, h: 438 }, rotate: 0 },
  },
  meta: {
    version: '1.0',
    image: 'items-spritesheet.png',
    css: 'items-card',
    size: {
      w: 2628,
      h: 2640,
    },
    scale: '1',
  },
};
